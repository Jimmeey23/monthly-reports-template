...
(function () {
  ...
  /* ------------------------------------------------------------------ styles */
  var CSS = [
    '.momp-panel{--momp-pad-x:20px;position:relative;margin:20px 0 24px;border:1px solid var(--border);',
    '  border-radius:var(--radius-lg);background:var(--bg-card);box-shadow:var(--shadow-sm);overflow:hidden}',
    '.momp-panel::before{content:"";position:absolute;top:0;left:0;right:0;height:3px;z-index:4;',
    '  background:linear-gradient(90deg,var(--primary),transparent)}',
    '.momp-head{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;flex-wrap:wrap;',
    '  padding:19px var(--momp-pad-x);background:linear-gradient(180deg,var(--bg-card),var(--bg-inset));',
    '  border-bottom:1px solid var(--border)}',
    '.momp-eyebrow{display:block;font-size:10px;letter-spacing:.12em;text-transform:uppercase;',
    '  font-weight:700;color:var(--text-subtle);margin-bottom:5px}',
    '.momp-title{margin:0;font-family:var(--font-serif);font-size:18px;line-height:1.2;font-weight:600;color:var(--text)}',
    '.momp-sub{margin:4px 0 0;max-width:720px;font-size:12.5px;line-height:1.45;color:var(--text-muted)}',
    '.momp-head-right{display:flex;align-items:center;gap:10px;flex-wrap:wrap}',
    '.momp-chip{display:inline-flex;align-items:center;gap:6px;padding:3px 10px;border-radius:999px;',
    '  border:0;background:var(--bg-inset);font-size:11px;font-weight:500;color:var(--text-muted)}',
    ...
    '.momp-toggle{display:inline-flex;align-items:center;gap:8px;padding:5px 12px;border-radius:999px;cursor:pointer;',
    '  border:1px solid var(--border);background:var(--bg-card);color:var(--text-muted);',
    '  font:inherit;font-size:11px;font-weight:600;transition:border-color .15s,color .15s,background .15s}',
    '.momp-toggle:hover{border-color:var(--primary);color:var(--text)}',
    ...
    '.momp-scroll{overflow-x:auto;border:1px solid var(--border);border-radius:var(--radius);background:var(--bg-card);box-shadow:var(--shadow-sm)}',
    'table.momp-table{width:100%;border-collapse:separate;border-spacing:0;font-size:12.5px;font-variant-numeric:tabular-nums}',
    'table.momp-table thead th{position:sticky;top:0;z-index:2;color:var(--text-muted);',
    '  background:linear-gradient(180deg,var(--bg-inset),color-mix(in srgb,var(--bg-inset) 72%,var(--bg-card)));',
    '  font-size:11px;letter-spacing:.04em;text-transform:uppercase;font-weight:800;white-space:nowrap;',
    '  text-align:right;padding:12px 14px;border-bottom:1px solid var(--border-strong)}',
    'table.momp-table thead th:first-child{text-align:left;padding-left:16px;left:0;z-index:3}',
    'table.momp-table thead th:last-child{padding-right:16px}',
    'table.momp-table tbody td{border-bottom:1px solid var(--border);padding:10px 14px;text-align:right;',
    '  background:var(--bg-card);white-space:nowrap;vertical-align:middle}',
    'table.momp-table tbody td:first-child{position:sticky;left:0;z-index:2;text-align:left;padding-left:16px;',
    '  border-right:1px solid var(--border);background:var(--bg-card)}',
    'table.momp-table tbody td:last-child{padding-right:16px}',
    ...
    '.momp-cell-value{display:block;font-size:12.5px;font-weight:600;color:var(--text);line-height:1.25}',
    '.momp-cell-delta{display:inline-flex;align-items:center;justify-content:center;min-width:56px;margin-top:4px;',
    '  padding:2px 7px;border-radius:999px;font-family:var(--font-mono);font-size:9.5px;font-weight:700;line-height:1.35}',
    '.momp-cell-delta.is-up{background:var(--good-soft);color:var(--good)}',
    '.momp-cell-delta.is-down{background:var(--bad-soft);color:var(--bad)}',
    '.momp-cell-delta.is-flat{background:var(--bg-inset);color:var(--text-muted)}',
    '.momp-cell.is-active{box-shadow:inset 0 0 0 1.5px var(--primary)}',
    ...
    'body.sp-report .mom-info-btn.is-open{',
  ...
  function groupedTable(rows, cols) {
    ...
    // A report only ships the columns its source data can support, so drop any
    // column no row carries rather than printing a column of dashes.
    cols = cols.filter(function (c) {
      return rows.some(function (r) { return r[c.key] !== null && r[c.key] !== undefined; });
    });
