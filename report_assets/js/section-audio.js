(function () {
  ...
  var prefix = RM.audioPrefix || (/supreme|bandra/i.test(window.location.pathname + ' ' + document.title) ? 'supreme' : 'kemps');
  // Some studios' narration sets skip a clip number after the opener.
  var clipSkipAfterFirst = RM.audioClipSkipAfterFirst === undefined
    ? /supreme|bandra/i.test(prefix)
    : !!RM.audioClipSkipAfterFirst;
  var audioIndex = 0;
  ...
  function audioUrl(fileName) {
    return new URL((RM.audioBase || '/audio/') + fileName, document.baseURI).href;
  }
  ...
  function attachButton(actions, title) {
    ...
    audioIndex += 1;
    var clipIndex = clipSkipAfterFirst && audioIndex > 1 ? audioIndex + 1 : audioIndex;
